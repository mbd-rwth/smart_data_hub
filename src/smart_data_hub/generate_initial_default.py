...
def generate_initial_default() -> None:
    ...
    layer_unqiue_lithologies = merged_df["layer_simplified_lithology"].explode().unique().tolist()
    ...
    for layer_unqiue_lithology in layer_unqiue_lithologies:
        ...
        merged_property_df = merge_property_value(
            input_df_for_merging,
            source_type="default",
            sampling_functions_by_property=sampling_functions_by_property,
        )
        ...
        if save_to_file:
            output_file_path = (
                files("smart_data_hub") / "dataset" / "rock_property" / "default" / f"{layer_unqiue_lithology}.yaml"
            )
            export2yaml(
                merged_property_df,
