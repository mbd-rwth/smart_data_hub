...
def test_referenced_geometry_files_exist(merged_df):
    ...
    for paths in geometry_surface_output_paths:
        if (paths is not None) and (str(paths) != "nan"):
            for path in paths.split(", "):
                surface_output_paths.append(os.path.join(*path.split(os.sep)[-3:]))

    geometry_vtk_paths = [
        os.path.join(*path.split(os.sep)[-3:]) for path in geometry_file_paths() if path.endswith(".vtk")
    ]
...
@pytest.mark.parametrize(
    "file_path",
    [Path(file_path) for file_path in geometry_file_paths() if file_path.endswith(".vtk")],
    ids=str,
)
def test_geometry_mesh_is_not_empty(file_path):
    rock_layer_ply = pv.read(file_path)

    assert not (rock_layer_ply.n_points == 0 and rock_layer_ply.n_cells == 0), (
...
def test_sourced_properties_do_not_have_empty_values(sourced_property_df):
    ...
    assert sourced_property_df[empty_mask].empty, f"Rows with missing data:\n{sourced_property_df[empty_mask]}"
...
def test_sourced_properties_do_not_have_invalid_values(sourced_property_df):
    ...
    assert sourced_property_df[is_invalid_mask].empty, (
        f"Rows with invalid entries:\n{sourced_property_df[is_invalid_mask]}"
    )
