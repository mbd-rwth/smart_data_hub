...
This folder summarizes geological information for siting regions, with one YAML file per region.
...
geometry_input:
  ...
  point_data_path: dataset/geometry/DE_Crystalline/input/points.csv
  orientation_data_path: dataset/geometry/DE_Crystalline/input/orientations.csv
Quaternary:
  source: weitkamp2021
  geological_time:
    eon: Phanerozoic
  ...
  property_path: dataset/rock_property/DE_Crystalline/Quaternary.yaml
  geometry_surface_output_path: [dataset/geometry/DE_Crystalline/output/Quaternary.vtk]
```
...
The missing values are filled based on the `simplified_lithology` entries in each stratigraphical layer. For instance, in the provided example, the simplified lithology of `Quaternary` is referenced as `[Sandstone, Conglomerate]`. In this case, the missing values are retrieved from the file `../rock_property/default/Sandstone.yaml` and `../rock_property/default/Conglomerate.yaml`.
